<?php
	...
	if(ISSET($_POST['edit_account_users'])){
		...
		$query = $conn->query("SELECT * FROM `users` WHERE `password` = '$password'") or die(mysqli_error($conn));
		$valid = $query->num_rows;
		if($valid > 0){
			$conn->query("UPDATE `users` SET `firstname` = '$firstname', `lastname` = '$lastname', `status` = '$status', `funcao` = '$funcao', `email` = '$email', `contactno` = '$contactno', `cpf` = '$cpf' WHERE `users_id` = '$_REQUEST[users_id]'") or die(mysqli_error($conn));
