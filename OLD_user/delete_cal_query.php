...
$stmt = $conn->prepare("SELECT locacao_id
                        ...
                        WHERE locacao_id IN (
                                SELECT locacao_id
                                FROM locacao 
                                WHERE room_id = ? AND checkin = ? AND checkin_time = ? AND mensagens_id = ? AND users_id = ?)
                        OR locacao_id IN (
                                SELECT locacao_id 
                                FROM locacao 
                                WHERE vehicle_id = ? AND checkin = ? AND checkin_time = ? AND mensagens_id = ? AND users_id = ?)
                        OR locacao_id IN (
                                SELECT locacao_id 
                                FROM locacao 
                                WHERE equip_id = ? AND checkin = ? AND checkin_time = ? AND mensagens_id = ? AND users_id = ?)");
$stmt->bind_param("issiiissiiissii", $room_id, $mysql_date, $timeFrom, $mensagens_id, $users_id, $vehicle_id, $mysql_date, $timeFrom, $mensagens_id, $users_id, $equip_id, $mysql_date, $timeFrom, $mensagens_id, $users_id);
$stmt->execute();
