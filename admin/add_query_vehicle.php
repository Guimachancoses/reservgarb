<?php
	...
	if(ISSET($_POST['add_vehicles'])){
		...
		$existingLabQuery = $conn->query("SELECT * FROM vehicles WHERE name = '$name' && model = '$model' && description = '$description' && photo = '$photo'");
		if ($existingLabQuery->num_rows > 0) {
			...
			$conn->query("INSERT INTO `vehicles` (name, model, description, photo, approver_id) VALUES('$name', '$model', '$description', '$photo', '$approver_id')") or die(mysqli_error($conn));
