...
$stmt = $conn->prepare("SELECT locacao_id FROM locacao WHERE locacao_id = (SELECT locacao_id FROM locacao WHERE room_id = ? && checkin = ? && checkin_time = ? && mensagens_id = ?)");
$stmt->bind_param("sssi", $room_id, $mysql_date, $timeFrom, $mensagens_id);
$stmt->execute();
