<?php
    ...
    if(ISSET($_POST['locacao_periodo'])){
        ...
        WHERE lc_period_id IN (
                SELECT lc_period_id
                FROM lc_period 
                WHERE room_id = ? AND checkin >= ? AND checkout <= ? AND checkin_time >= ? AND checkout_time <= ? and mensagens_id != 4)
        OR lc_period_id IN (
                SELECT lc_period_id 
                FROM lc_period 
                WHERE vehicle_id = ? AND checkin >= ? AND checkout <= ? AND checkin_time >= ? AND checkout_time <= ? and mensagens_id != 4)
        OR lc_period_id IN (
                SELECT lc_period_id 
                FROM lc_period 
                WHERE equip_id = ? AND checkin >= ? AND checkout <= ? AND checkin_time >= ? AND checkout_time <= ? and mensagens_id != 4)");
        $verif->bind_param("issssissssissss", $room_id, $mysql_dateIn, $mysql_dateOut, $timeFrom, $timeTo, $vehicle_id, $mysql_dateIn, $mysql_dateOut, $timeFrom, $timeTo, $equip_id, $mysql_dateIn, $mysql_dateOut, $timeFrom, $timeTo);
        $verif->execute();
        $verif->store_result();
        ...
        if ($valid > 0) {
            ...
            //Define a resposta
            echo "<script>alert('Solicitação de reserva enviada com sucesso!'); window.location.href = 'reservlab.php';</script>";
        }
    }
?>