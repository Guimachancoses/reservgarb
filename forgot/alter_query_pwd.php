...
<?php
	...
	if(ISSET($_POST['alter_query_pwd'])){
		...
		$codigo = AntiSqlInjection(trim($_POST['codigo']), $conn);
		...
		// Verifica se email e código estão corretos
		...
		if ($valid > 0) {
			...
			$stmt->execute();
			$stmt->close();
			echo "<script>alert('Código de verificação expirado.\\nCaso o erro persista contacte o administrador.'); window.location.href = 'index.php'</script>";
		}
	}
?>