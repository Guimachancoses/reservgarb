<?php
	...
	if(ISSET($_POST['add_subject'])){
		...
		echo "console.log($nm_disciplina);";
		echo "console.log($curso);";
		echo "console.log($semestre);";
		echo "console.log($users_id);";
		...
		if($valid > 0){
			...
			$stmt = $conn->prepare("INSERT INTO disciplinas (users_id, curso_id, semester_id, nm_disciplina, qtd_users, date) VALUES (?, ?, ?, ?, ?, ?)");
			$stmt->bind_param("iiisss", $users_id, $curso_id, $semester_id, $nm_disciplina, $qtd_users, $date);
			$stmt->execute();
			$stmt->close();	

			$conn->query("INSERT INTO `activities` set mensagens_id = 18, users_id = '$_SESSION[users_id]'") or die(mysqli_error());
