...
<?php
	...
	if(ISSET($_POST['alter_query_account'])){
		...
		$queryrl = $conn->prepare("SELECT u.password FROM `users` as u	WHERE u.funcao = 'Usuário' && u.users_id = ?") or die(mysqli_error($conn));
		...
		if ($fetchap) {
			if (password_verify($password, $storedHashedPasswordAP)) {
				...
				$urlBase = ('../user/reservlab.php?alter-account.php');
				...
				$conn->query("UPDATE `users` SET `contactno` = '$contactno', `password` = '$hashedPassword', `time_uppass` = '$time_uppass' WHERE `users_id` = '$users_id'") or die(mysqli_error());
